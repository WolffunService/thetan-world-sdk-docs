...
namespace Wolffun.RestAPI.JWT
{
    ...
    public static class JsonWebToken
    {
        ...
        {
            var jsonPayload = Decode(token, Encoding.UTF8.GetBytes(key), verify);
            var claims = JsonSerializer.Deserialize<Dictionary<string, object>>(jsonPayload);
            return claims;
        }

        public static bool IsTokenExpire(string token, int offsetInSecond)
        {
            if (string.IsNullOrEmpty(token))
                return true;
            
            //pre process to extract data
            var parts = token.Split('.');
            if (parts.Length != 3)
            {
                throw new ArgumentException("Token must consist from 3 delimited by dot parts");
            ...
            if (payloadData.ContainsKey("exp") && payloadData["exp"] != null)
            {
                ...
                {
                    //can not extract expire data --> need to return token has expired
                    return true;
                    //throw new SignatureVerificationException("Claim 'exp' must be an integer.");
                }

                var secondsSinceEpoch = Math.Round((DateTime.UtcNow - UnixEpoch).TotalSeconds);
                return secondsSinceEpoch + offsetInSecond >= exp;
            }

            //don't have key expire time??? --> sth wrong with token, need to return token has expired
            return true;
        }

        public static DateTime GetTimeExpire(string token)
        {
            if (string.IsNullOrEmpty(token))
                return default;
